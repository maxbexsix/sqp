...
IS
  ...
  PROCEDURE GeneratePublicSynonyms(SchemaName VARCHAR2);
END;
/
...
BEGIN
  ...
  ELSE
     FOR O IN (SELECT OBJECT_TYPE, OBJECT_NAME
                 FROM ALL_OBJECTS                 
                WHERE OBJECT_TYPE = UPPER(ObjectType)
  ...
  FOR C IN (SELECT OBJECT_NAME
             ...
             WHERE OWNER = UPPER(SchemaName)
               AND (OBJECT_TYPE IN ('TABLE',
                                    ...
                                    'PACKAGE',
                                    'SEQUENCE')
                OR (OBJECT_TYPE IN ('TYPE') AND OBJECT_NAME NOT LIKE 'SYS%')))
  LOOP
    ExecSQL('CREATE OR REPLACE PUBLIC SYNONYM ' || C.OBJECT_NAME || ' FOR ' || SchemaName || '.' || C.OBJECT_NAME);
  END LOOP;
END;
...
GRANT all ON dbo.DbTools TO PUBLIC;
